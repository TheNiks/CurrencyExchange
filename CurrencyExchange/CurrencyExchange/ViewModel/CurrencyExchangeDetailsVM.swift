//
//  CurrencyExchangeDetailsVM.swift
//  CurrencyExchange
//
...
class CurrencyExchangeDetailsVM {
    ...
    func getHistoricalCurrencyData(fromSymbol: String, toSymbol: String, valueToConvert: String, convertedLatestValue: String) {
        ...
        historicalModel.append(HistoricalDataModel(fromCurrencySymbol: fromSymbol, fromCurrencyValue: valueToConvert, toCurrencySymbol: toSymbol, toCurrencyValue: convertedLatestValue, dateString: dateStr))
        ...
        for dateString in datesArray {
            dispatchGroup.enter()
            NetworkServiceManger.shared.getDataResponse(urlString: dateString, queryItems: queryItemsDict, completionBlock: { [weak self] result in
                ...
                case .success(let data) :
                    if let rates = data[StringConstants.ratesKey] as? [String: Double] {
                        if let fromValue = rates[fromSymbol], let toValue = rates[toSymbol], let value = Double(valueToConvert) {
                            ...
                            historicalModel.append(HistoricalDataModel(fromCurrencySymbol: fromSymbol, fromCurrencyValue: valueToConvert, toCurrencySymbol: toSymbol, toCurrencyValue: convertedValue, dateString: dateString))
                        }
                    }
                }
                dispatchGroup.leave()
